...
<div class="yoo-zoo <?php echo $css_class; ?> <?php echo $css_class.'-'.$this->category->alias; ?>">
	...
	<?php if ($this->params->get('template.show_title') || $this->params->get('template.show_description') || $this->params->get('template.show_image')) : ?>
		...
		<h1 class="<?php echo 'uk-text-'.$this->params->get('template.alignment'); ?>"><?php echo $this->category->name.' '.JText::_('Archive'); ?></h1>
		<?php endif; ?>
	...
	<?php
		...
		if (count($this->items)) {
			...
			$subtitle = $this->category->getParams()->get('content.items_subtitle');
