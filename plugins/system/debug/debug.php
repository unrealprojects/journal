			...
			if ($timings && isset($timings[$id * 2 + 1]))
			{
				...
				if (isset($duplicates[$did]))
				{
					...
					$htmlQuery = '<div class="alert alert-error">' . JText::_('PLG_DEBUG_QUERY_DUPLICATES') . ': ' . implode('&nbsp; ', $dups) . '</div>'
						. '<pre class="alert " title="' . JHtml::tooltipText('PLG_DEBUG_QUERY_DUPLICATES_FOUND') . '">' . $text . '</pre>';
				}
				else
				{
			...
			foreach ($tr as $k => $td)
			{
				...
				elseif ($k == 'key')
				{
					if ($td === 'NULL')
					{
						...
						$html[] = '<td><strong>' . '<span class="dbg-warning " title="' . JHtml::tooltipText('PLG_DEBUG_WARNING_NO_INDEX_DESC') . '">'
				...
				elseif ($k == 'Extra')
				{
					...
					$htmlTdWithWarnings = str_replace('Using&nbsp;filesort', '<span class="dbg-warning " title="'
